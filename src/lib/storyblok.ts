...
export function useStoryblok<T extends StoryblokComponent<string>>(
  originalStory: StoryData<T>,
  preview: boolean
) {
  let [story, setStory] = useState<StoryData<T>>(originalStory);
  ...
  function addBridge(callback: VoidFunction) {
